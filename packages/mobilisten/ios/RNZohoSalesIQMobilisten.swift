...
class RNZohoSalesIQMobilisten: RCTEventEmitter, ZohoSalesIQDelegate, ZohoSalesIQChatDelegate, ZohoSalesIQFAQDelegate, ZohoSalesIQKnowledgeBaseDelegate {
    ...
    let RE_REGISTER_PUSH = "RE_REGISTER_PUSH"
    let TYPE_OPEN = "OPEN"
    ...
    func shouldReRegisterPushNotification() {
        if hasSIQEventListener {
            sendRCTEvent(withName:ZSIQ_EVENT_LISTENER, body: RNZohoSalesIQMobilisten.getEventEmitterObject(RE_REGISTER_PUSH, body: nil))
        }
    }
    ...
    // MARK: - initialize
    @MainActor @objc
    func initialize(_ configuration: [String: Any], resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
        RNZohoSalesIQMobilisten.mainThread {
            let appKey = configuration["appKey"] as? String ?? ""
            let accessKey = configuration["accessKey"] as? String ?? ""
            ZohoSalesIQ.setPlatform(platform: "ReactNative")
            ZohoSalesIQ.initWithAppKey(appKey, accessKey: accessKey, authProvider: nil) { error in
                if error == nil {
                    resolve(true)
                } else {
                    reject("500" ,"❌ MOBILISTEN » INITIALIZATION FAILED", nil)
                }
            }
            ZohoSalesIQ.delegate = self
            ZohoSalesIQ.Chat.delegate = self
    ...
    // MARK: - setTitle
    @MainActor @objc
    func setChatConversationTitle(_ online: String?, offline: String?) {
        ZohoSalesIQ.Chat.setTitle(online: online, offline: offline)
    }
    
    @MainActor @objc
    func getCommunicationMode(_ resolve: RCTPromiseResolveBlock, rejecter reject: RCTPromiseRejectBlock) {
        let mode = ZohoSalesIQ.getCommunicationMode
        switch mode {
        case .chat:
            resolve("CHAT")
        case .call:
    ...
    // MARK: - fetchDepartments
    @MainActor @objc
    func fetchDepartments(_ resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
        ZohoSalesIQ.Conversation.getDepartments { error, departments in
            if let error = error {
                ...
                for department in departments {
                    ...
                    let departmentDict: [String: Any] = ["name": department.name, "id": department.id, "communicationMode": mode, "available": department.available]
    ...
    // MARK: - setAttributes
    @MainActor @objc
    func setAttributes(_ attributes: [String: Any], resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
